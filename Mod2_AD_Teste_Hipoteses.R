...
#Gera v.a. que segue uma distribuição F (não normal) com n = 15, 2 graus de liberdade
#no numerados e 10 graus de liberdade no denominador
...
shapiro.test(va_normal) #87% de chance de errar ao rejeitar H0
shapiro.test(va_nao_normal) #0.2% de chance de errar ao rejeitar H0
...
options(scipen=666) #Esse comando remove a notação científica

2*pt(q = t, df = gl) # o p valor foi de 0.00000002403372. Entao a probabilidade de rejeitar H0 e estar errado é de 0.000002403372%. Então rejeiitamos H0
...
#alternative = 'two.sided' . realiza o teste bilateral. Ou seja, para os dois lados da curva
...
#install.packages().
#Uma vez instalada não há mais necessidade de instalar novamente. Basta carregar com
#o comando library()
...
hist(diferenca)
shapiro.test(diferenca) #O p test foi de 0.7485. Ou seja, há uma prob. de 75% de estar errado ao rejeitar H0
...
#O p valor foi de 0.1759 e o alfa é 0.10. Então, nao temos evidencias para rejeitar a hipotese nula H0
...
tcrítico_teste_t_pareado <- -qt(p = 0.9, df = 19) #Devido ao teste ser unilateral a
#esquerda a distribuição t ser simétrica, nossa estatística de teste será negativa
#Observe que o t calculado é maior que o t critico. Como estamos em um teste unilateral
#a esquerda o t calculado
...
geom_density(alpha=0.4)+
  xlim(-50,300)
#É bastante comum também analisarmos a variabilidade nas distintas populações com
#uso de boxplot
...
anova <- aov(Gastos~ #Variável resposta
               Estado_Civil, #Fator que queremos testar se exerce influencia na variávelresposta
             data = dados_anova)
